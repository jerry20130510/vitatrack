...
const CONSTANTS = {
    ...
    MESSAGES: {
        LOAD_ERROR: '載入失敗',
        LOAD_ERROR_LIST: '載入文章列表失敗',
        LOAD_ERROR_TITLE: '無法載入文章',
        LOAD_ERROR_DESC: '請檢查網路連線再試一次',
        RELOAD_BTN: '重新載入',
        READ_MORE: '閱讀更多',
        PAGINATION_INFO: (start, end, total) => `顯示第 ${start}-${end} 項，共 ${total} 項`
    }
};
...
document.querySelectorAll('.nav-link').forEach(link => {
    link.addEventListener('click', (e) => {
        e.preventDefault();
        document.querySelectorAll('.nav-link').forEach(l => l.classList.remove(CONSTANTS.CSS_CLASSES.ACTIVE));
...
function loadArticles() {
    ...
    el('articles-container').style.display = CONSTANTS.DISPLAY.NONE;
    toggle(el('pagination'), false);
    
    const params = new URLSearchParams({ page: currentPage, size: CONSTANTS.PAGE_SIZE });
    if (currentCategory) params.append('category', currentCategory);
    if (currentAuthor) params.append('authorSlug', currentAuthor);

    fetch(`${API_BASE}/articles?${params}`)
        .then(response => {
            if (!response.ok) throw new Error(CONSTANTS.MESSAGES.LOAD_ERROR);
            return response.json();
        })
        .then(result => {
            if (!result.success) {
                throw new Error(result.errMsg || CONSTANTS.MESSAGES.LOAD_ERROR_LIST);
            }
            const data = result.data;
            renderArticles(data.articles);
            renderPagination(data);
            el('articles-container').style.display = CONSTANTS.DISPLAY.FLEX;
        })
        .catch(error => {
            console.error('[Load] error:', error);
            const container = el('articles-container');
            if (container) {
                container.style.display = CONSTANTS.DISPLAY.BLOCK;
                container.innerHTML = `
                    <div class="col-12">
                        <div class="error-display">
                            <i class="ri-wifi-off-line error-icon"></i>
                            <h3 class="mn-title error-title">${CONSTANTS.MESSAGES.LOAD_ERROR_TITLE}</h3>
                            <p class="error-message">${CONSTANTS.MESSAGES.LOAD_ERROR_DESC}</p>
                            <button onclick="loadArticles()" class="mn-btn-2"><span>${CONSTANTS.MESSAGES.RELOAD_BTN}</span></button>
                        </div>
                    </div>
                `;
            }
                ...
                <div class="blog-info blog-card-clickable" onclick="location.href='blog-detail.html?slug=${a.titleSlug}'">
                    <figure class="blog-img blog-card-image">
                        <a href="blog-detail.html?slug=${a.titleSlug}">
                            <img src="${a.imageUrl || CONSTANTS.PLACEHOLDER_IMAGE}" 
                                 onerror="this.onerror=null; this.src='${CONSTANTS.PLACEHOLDER_IMAGE}';"
                    ...
                    <div class="detail blog-card-detail">
                        ...
                        <div class="more-info">
                            <a href="blog-detail.html?slug=${a.titleSlug}">${CONSTANTS.MESSAGES.READ_MORE}<i class="ri-arrow-right-double-line"></i></a>
                        </div>
                    </div>
                </div>
            </div>
...
function renderPagination(data) {
    ...
    el('pagination').style.display = CONSTANTS.DISPLAY.FLEX;
    const start = data.page * data.size + 1;
    const end = Math.min((data.page + 1) * data.size, data.totalElements);
    el('pagination-info').textContent = CONSTANTS.MESSAGES.PAGINATION_INFO(start, end, data.totalElements);
